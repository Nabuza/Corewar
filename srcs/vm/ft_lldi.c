...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/08/03 11:56:52 by nolivier          #+#    #+#             */
/*   Updated: 2017/09/28 11:20:48 by nolivier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "vm.h"

static void	ft_print_lldi(t_process *process, unsigned char **params,
	...
	unsigned char *memory)
{
	...
	while (++i < REG_SIZE)
		...
		memory[(process->proc_counter - (-s1 % IDX_MOD) + i) % MEM_SIZE];
	ft_modif_carry_reg(process, params[2], 14);
}

int			ft_lldi(t_settings *sets, t_process *process, t_process **begin)
{
	unsigned char	ocp;
	unsigned int	counter;
	unsigned char	*params[3];

	if (++process->cycle_count < (g_op_tab + 13)->cycles)
		return (1);
	...
	ft_print_lldi(process, params, sets, counter);
